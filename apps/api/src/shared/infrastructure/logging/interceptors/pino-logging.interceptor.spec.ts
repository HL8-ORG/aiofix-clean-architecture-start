  ...
  beforeEach(() => {
    ...
    mockExecutionContext = {
      ...
      getArgs: jest.fn().mockReturnValue([
        { method: 'GET', requestId: 'req-123', tenantId: 'tenant-123', userId: 'user-456' },
        { statusCode: 200 },
        { id: 1, name: 'test' },
      ]),
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
        next: (result) => {
          ...
          expect(logger.info).toHaveBeenCalledWith(
            expect.stringMatching(/Method Success: TestController\.testMethod - \d+ms/),
            LogContext.BUSINESS,
            expect.objectContaining({
              methodId: expect.any(String),
              className: 'TestController',
    ...
    it('should generate unique method ID for each call', (done) => {
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
        next: () => {
          ...
          interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
            next: () => {
              const secondDebugCall = (logger.debug as jest.Mock).mock.calls[1];
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe({
        ...
        error: (error) => {
          ...
          expect(logger.error).toHaveBeenCalledWith(
            expect.stringMatching(/Method Error: TestController\.testMethod - \d+ms/),
            LogContext.BUSINESS,
            expect.objectContaining({
              methodId: expect.any(String),
              className: 'TestController',
