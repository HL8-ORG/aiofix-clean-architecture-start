...
describe('MemoryCacheService', () => {
  ...
  describe('cache strategies', () => {
    ...
    it('should use LFU strategy for eviction', async () => {
      const config: MemoryCacheConfig = { ...mockConfig, maxSize: 2, defaultStrategy: CacheStrategy.LFU };
      const module = await Test.createTestingModule({
        providers: [
          MemoryCacheService,
          { provide: 'MEMORY_CACHE_CONFIG', useValue: config },
    ...
    it('should use FIFO strategy for eviction', async () => {
      const config: MemoryCacheConfig = { ...mockConfig, maxSize: 2, defaultStrategy: CacheStrategy.FIFO };
      const module = await Test.createTestingModule({
        providers: [
          MemoryCacheService,
          { provide: 'MEMORY_CACHE_CONFIG', useValue: config },
      ...
      const module = await Test.createTestingModule({
        providers: [
          ...
          { provide: 'MEMORY_CACHE_CONFIG', useValue: mockConfig },
          {
            provide: 'ICacheKeyFactory',
            useValue: {
              create: () => { throw new Error('Mock error'); },
              toString: () => 'test',
              parse: () => ({ key: 'test' }),
            },
          },
