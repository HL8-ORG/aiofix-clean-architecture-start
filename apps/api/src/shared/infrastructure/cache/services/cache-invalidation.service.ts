 ...
 * @description
 * 缓存失效结果接口，定义失效操作的结果信息。
 */
export interface InvalidationResult {
  /** 失效的键数量 */
  readonly invalidatedKeys: number;
  /** 失效的命名空间数量 */
  readonly invalidatedNamespaces: number;
  /** 失效的标签数量 */
  readonly invalidatedTags: number;
  /** 失效的键列表 */
...
export class CacheInvalidationService {
  ...
  setCacheService(cacheService: ICacheService): void {
    ...
    this.logger.info('Cache service set for invalidation service', LogContext.CACHE);
  }
   ...
   */
  addRule(rule: Omit<InvalidationRule, 'id' | 'createdAt' | 'updatedAt'>): boolean {
    try {
      ...
      this.logger.info(`Added invalidation rule: ${rule.name}`, LogContext.CACHE);
      this.emitEvent('rule_added', { rule: fullRule });
      return true;
    } catch (error) {
      this.logger.error(`Failed to add invalidation rule: ${rule.name}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  removeRule(ruleId: string): boolean {
    try {
      if (!this.rules.has(ruleId)) {
        this.logger.warn(`Invalidation rule not found: ${ruleId}`, LogContext.CACHE);
        return false;
      }
      ...
      this.logger.info(`Removed invalidation rule: ${rule.name}`, LogContext.CACHE);
      this.emitEvent('rule_removed', { rule });
      return true;
    } catch (error) {
      this.logger.error(`Failed to remove invalidation rule: ${ruleId}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
   ...
   * @returns {boolean} 是否成功
   */
  updateRule(ruleId: string, updates: Partial<Omit<InvalidationRule, 'id' | 'createdAt'>>): boolean {
    try {
      if (!this.rules.has(ruleId)) {
        this.logger.warn(`Invalidation rule not found: ${ruleId}`, LogContext.CACHE);
        return false;
      ...
      this.logger.info(`Updated invalidation rule: ${updatedRule.name}`, LogContext.CACHE);
      this.emitEvent('rule_updated', { rule: updatedRule });
      return true;
    } catch (error) {
      this.logger.error(`Failed to update invalidation rule: ${ruleId}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  async invalidate(
    ...
    try {
      ...
      this.logger.info(`Cache invalidation completed: ${result.invalidatedKeys} keys, ${result.executionTime}ms`, LogContext.CACHE);
      ...
      this.logger.error(`Cache invalidation failed: ${error.message}`, LogContext.CACHE, undefined, error);
  ...
  async invalidateByRule(
    ...
    if (!rule.enabled) {
      this.logger.warn(`Invalidation rule is disabled: ${rule.name}`, LogContext.CACHE);
      return {
        invalidatedKeys: 0,
        invalidatedNamespaces: 0,
        invalidatedTags: 0,
    ...
    if (rule.condition && !this.evaluateCondition(rule.condition, context)) {
      this.logger.debug(`Invalidation rule condition not met: ${rule.name}`, LogContext.CACHE);
      return {
        invalidatedKeys: 0,
        invalidatedNamespaces: 0,
        invalidatedTags: 0,
    ...
    if (rule.dependencies && rule.dependencies.length > 0) {
      for (const depId of rule.dependencies) {
        ...
        if (!depRule || !depRule.enabled) {
          this.logger.warn(`Dependency rule not found or disabled: ${depId}`, LogContext.CACHE);
          return {
            invalidatedKeys: 0,
            invalidatedNamespaces: 0,
            invalidatedTags: 0,
  ...
  private async invalidateExact(keys: string[]): Promise<string[]> {
    ...
    for (const key of keys) {
      try {
        ...
        this.logger.warn(`Failed to invalidate exact key: ${key}`, LogContext.CACHE, undefined, error);
      }
    }
  ...
  private async invalidatePrefix(prefixes: string[]): Promise<string[]> {
    ...
    for (const prefix of prefixes) {
      try {
        ...
        this.logger.warn(`Failed to invalidate prefix: ${prefix}`, LogContext.CACHE, undefined, error);
      }
    }
  ...
  private async invalidateWildcard(patterns: string[]): Promise<string[]> {
    ...
    for (const pattern of patterns) {
      try {
        ...
        // 这里需要缓存服务提供键列表查询功能
        // 简化实现
        this.logger.debug(`Wildcard invalidation pattern: ${pattern}`, LogContext.CACHE);
      } catch (error) {
        this.logger.warn(`Failed to invalidate wildcard pattern: ${pattern}`, LogContext.CACHE, undefined, error);
      }
    }
  ...
  private async invalidateRegex(patterns: string[]): Promise<string[]> {
    ...
    for (const pattern of patterns) {
      try {
        ...
        // 这里需要缓存服务提供键列表查询功能
        // 简化实现
        this.logger.debug(`Regex invalidation pattern: ${pattern}`, LogContext.CACHE);
      } catch (error) {
        this.logger.warn(`Failed to invalidate regex pattern: ${pattern}`, LogContext.CACHE, undefined, error);
      }
    }
  ...
  private async invalidateTags(tags: string[]): Promise<string[]> {
    ...
    for (const tag of tags) {
      try {
        ...
        this.logger.warn(`Failed to invalidate tag: ${tag}`, LogContext.CACHE, undefined, error);
      }
    }
  ...
  private async invalidateNamespaces(namespaces: string[]): Promise<string[]> {
    ...
    for (const namespace of namespaces) {
      try {
        ...
        this.logger.warn(`Failed to invalidate namespace: ${namespace}`, LogContext.CACHE, undefined, error);
      }
    }
  ...
  private async invalidateBatch(targets: string[]): Promise<string[]> {
    ...
    for (let i = 0; i < targets.length; i += batchSize) {
      ...
      try {
        ...
        this.logger.warn(`Failed to invalidate batch ${i / batchSize + 1}`, LogContext.CACHE, undefined, error);
      }
    }
  ...
  private evaluateCondition(condition: string, context?: Record<string, any>): boolean {
    try {
      ...
      this.logger.warn(`Failed to evaluate condition: ${condition}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  private emitEvent(type: string, data: any): void {
    if (this.config.enableEvents) {
      try {
        ...
        this.logger.warn(`Failed to emit invalidation event: ${type}`, LogContext.CACHE, undefined, error);
      }
    }
  }
    ...
    if (this.config.monitoringInterval && this.config.monitoringInterval > 0) {
      this.monitoringTimer = setInterval(async () => {
        try {
          ...
          this.logger.error('Invalidation monitoring failed', LogContext.CACHE, undefined, error);
        }
      }, this.config.monitoringInterval);

      this.logger.info(`Started invalidation monitoring, interval: ${this.config.monitoringInterval}ms`, LogContext.CACHE);
    }
  }
  ...
  private async performMonitoring(): Promise<void> {
    try {
      ...
      this.logger.debug(`Active invalidation rules: ${activeRules.length}`, LogContext.CACHE);
    } catch (error) {
      this.logger.error('Invalidation monitoring execution failed', LogContext.CACHE, undefined, error);
    }
  }
}
