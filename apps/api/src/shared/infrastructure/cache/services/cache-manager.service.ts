...
import type { CacheOptions, CacheKey, CacheStats, CacheHealth } from '../interfaces/cache.interface';
...
export class CacheManagerService implements ICacheService {
  ...
  async get<T = any>(key: CacheKey): Promise<T | null> {
    ...
    try {
      ...
      this.logger.error(`Error getting cache value: ${key.key}`, LogContext.CACHE, undefined, error);
   ...
   */
  async set<T = any>(key: CacheKey, value: T, options?: Partial<CacheOptions>): Promise<boolean> {
    const startTime = Date.now();
    ...
    try {
      ...
      for (const layer of this.getSortedLayers()) {
        ...
        try {
          const layerSuccess = await layer.service.set(key, value, { ...layer.options, ...mergedOptions });
          if (layerSuccess) {
            success = true;
            this.emitEvent('cache_set', { key, layer: layer.name, value });
          }
        } catch (error) {
          this.logger.warn(`Failed to set cache in layer ${layer.name}: ${key.key}`, LogContext.CACHE, undefined, error);
        }
      }
      ...
      this.logger.error(`Error setting cache value: ${key.key}`, LogContext.CACHE, undefined, error);
      this.emitEvent('cache_error', { key, error });
      return false;
    }
  }
    ...
    try {
      ...
      for (const layer of this.getSortedLayers()) {
        ...
        try {
          ...
          this.logger.warn(`Failed to delete cache from layer ${layer.name}: ${key.key}`, LogContext.CACHE, undefined, error);
        }
      }
      ...
      this.logger.error(`Error deleting cache value: ${key.key}`, LogContext.CACHE, undefined, error);
      this.emitEvent('cache_error', { key, error });
      return false;
    }
  }
  ...
  async exists(key: CacheKey): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`Error checking cache existence: ${key.key}`, LogContext.CACHE, undefined, error);
      this.emitEvent('cache_error', { key, error });
      return false;
    }
  }
    ...
    try {
      ...
      for (const layer of this.getSortedLayers()) {
        ...
        try {
          ...
          this.logger.warn(`Failed to clear cache layer ${layer.name}`, LogContext.CACHE, undefined, error);
        }
      }
      ...
      for (const [name, layer] of this.layers) {
        if (layer.enabled) {
          try {
            ...
            this.logger.warn(`Failed to get stats for layer ${name}`, LogContext.CACHE, undefined, error);
  ...
  async getHealth(): Promise<CacheHealth> {
    try {
      ...
      this.logger.error('Error getting cache health', LogContext.CACHE, undefined, error);
      return {
        healthy: false,
        connected: false,
        responseTime: 0,
  ...
  addLayer(config: CacheLayerConfig): boolean {
    try {
      if (this.layers.has(config.name)) {
        this.logger.warn(`Cache layer ${config.name} already exists, replacing...`, LogContext.CACHE);
      }
      ...
      this.logger.info(`Added cache layer: ${config.name} with priority ${config.priority}`, LogContext.CACHE);
      this.emitEvent('layer_added', { layer: config.name, priority: config.priority });
      return true;
    } catch (error) {
      this.logger.error(`Failed to add cache layer: ${config.name}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  removeLayer(name: string): boolean {
    try {
      if (!this.layers.has(name)) {
        this.logger.warn(`Cache layer ${name} does not exist`, LogContext.CACHE);
        return false;
      }
      ...
      this.logger.error(`Failed to remove cache layer: ${name}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  enableLayer(name: string, enabled: boolean): boolean {
    try {
      ...
      this.logger.info(`${enabled ? 'Enabled' : 'Disabled'} cache layer: ${name}`, LogContext.CACHE);
      this.emitEvent('layer_toggled', { layer: name, enabled });
      return true;
    } catch (error) {
      this.logger.error(`Failed to toggle cache layer: ${name}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  async resetStats(): Promise<void> {
    try {
      ...
      for (const [name, layer] of this.layers) {
        if (layer.enabled) {
          try {
            ...
            this.logger.warn(`Failed to reset stats for layer ${name}`, LogContext.CACHE, undefined, error);
          }
        }
      }
      ...
      this.logger.error('Failed to reset cache stats', LogContext.CACHE, undefined, error);
    }
  }
   ...
   */
  private async promoteToHigherLayer<T>(key: CacheKey, value: T, currentPriority: number): Promise<void> {
    try {
      const higherLayers = Array.from(this.layers.values())
        .filter(layer => layer.enabled && !layer.readOnly && layer.priority < currentPriority)
        .sort((a, b) => a.priority - b.priority);

      for (const layer of higherLayers) {
        try {
          await layer.service.set(key, value);
          this.emitEvent('cache_promoted', { key, fromPriority: currentPriority, toPriority: layer.priority, layer: layer.name });
        } catch (error) {
          this.logger.warn(`Failed to promote cache to layer ${layer.name}: ${key.key}`, LogContext.CACHE, undefined, error);
        }
      }
    } catch (error) {
      this.logger.error(`Error promoting cache: ${key.key}`, LogContext.CACHE, undefined, error);
    }
  }
  ...
  private emitEvent(type: string, data: any): void {
    if (this.config.enableEvents) {
      try {
        ...
        this.logger.warn(`Failed to emit cache event: ${type}`, LogContext.CACHE, undefined, error);
      }
    }
  }
    ...
    if (this.config.monitoringInterval && this.config.monitoringInterval > 0) {
      this.monitoringTimer = setInterval(async () => {
        try {
          ...
          this.logger.error('Cache monitoring failed', LogContext.CACHE, undefined, error);
        }
      }, this.config.monitoringInterval);

      this.logger.info(`Started cache monitoring, interval: ${this.config.monitoringInterval}ms`, LogContext.CACHE);
    }
  }
    ...
    if (this.config.cleanupInterval && this.config.cleanupInterval > 0) {
      this.cleanupTimer = setInterval(async () => {
        try {
          ...
          this.logger.error('Cache cleanup failed', LogContext.CACHE, undefined, error);
        }
      }, this.config.cleanupInterval);

      this.logger.info(`Started cache cleanup, interval: ${this.config.cleanupInterval}ms`, LogContext.CACHE);
    }
  }
  ...
  private async performMonitoring(): Promise<void> {
    try {
      ...
      this.logger.error('Cache monitoring execution failed', LogContext.CACHE, undefined, error);
    }
  }
  ...
  private async performCleanup(): Promise<void> {
    try {
      ...
      for (const layer of this.getSortedLayers()) {
        if (layer.enabled && !layer.readOnly) {
          try {
            ...
            this.logger.warn(`Failed to cleanup layer ${layer.name}`, LogContext.CACHE, undefined, error);
          }
        }
      }
      ...
      this.logger.error('Cache cleanup execution failed', LogContext.CACHE, undefined, error);
    }
  }
}
