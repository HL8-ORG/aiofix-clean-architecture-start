...
export class MemoryCacheService implements ICacheService, OnModuleDestroy {
  ...
  constructor(
    @Inject('MEMORY_CACHE_CONFIG') private readonly userConfig: MemoryCacheConfig,
    ...
    this.logger.info(`MemoryCacheService initialized with config: ${JSON.stringify(this.config)}`, LogContext.CACHE);
  }
  ...
  async get<T = any>(key: CacheKey): Promise<T | null> {
    try {
      ...
      this.logger.error(`Error getting cache value: ${error.message}`, LogContext.CACHE, undefined, error);
      this.updateStats(false);
      return null;
    }
  }
  ...
  async set<T = any>(key: CacheKey, value: T, options?: Partial<CacheOptions>): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error setting cache value: ${error.message}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  async delete(key: CacheKey): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error deleting cache value: ${error.message}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  async exists(key: CacheKey): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error checking cache key existence: ${error.message}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  async clear(namespace?: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error clearing cache: ${error.message}`, LogContext.CACHE, undefined, error);
      return false;
    }
  }
  ...
  async getHealth(): Promise<CacheHealth> {
    ...
    try {
      ...
      return {
        healthy: retrieved !== null && retrieved.timestamp === testValue.timestamp,
        connected: true,
        responseTime,
        lastCheck: Date.now(),
      };
  ...
  private cleanupExpiredEntries(): void {
    ...
    if (keysToDelete.length > 0) {
      this.logger.debug(`Cleaned up ${keysToDelete.length} expired entries`, LogContext.CACHE);
    }
  }
