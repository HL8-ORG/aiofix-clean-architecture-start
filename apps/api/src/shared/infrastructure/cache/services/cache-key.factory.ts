...
export class CacheKeyFactory {
   ...
   * @returns {string} 缓存键
   */
  createKey(namespace: string, type: string, id: string, options?: CacheKeyOptions): string {
    const parts = [
    ...
    if (this.config.enableValidation && key.length > this.config.maxKeyLength) {
      this.logger.warn(`Cache key too long: ${key.length} > ${this.config.maxKeyLength}`, LogContext.CACHE);
   ...
   * @returns {string} 缓存键
   */
  createUserKey(userId: string, tenantId: string, type: string, id: string, options?: CacheKeyOptions): string {
   ...
   * @returns {string} 缓存键
   */
  createTenantKey(tenantId: string, type: string, id: string, options?: CacheKeyOptions): string {
   ...
   * @returns {string} 缓存键
   */
  createSessionKey(sessionId: string, userId: string, tenantId: string, options?: CacheKeyOptions): string {
   ...
   * @returns {string} 缓存键
   */
  createEventKey(eventId: string, aggregateId: string, eventType: string, options?: CacheKeyOptions): string {
   ...
   * @returns {string} 缓存键
   */
  createAggregateKey(aggregateId: string, aggregateType: string, options?: CacheKeyOptions): string {
   ...
   * @returns {string} 缓存键
   */
  createSnapshotKey(aggregateId: string, version: number, options?: CacheKeyOptions): string {
   ...
   * @returns {string} 缓存键
   */
  createProjectionKey(projectionName: string, projectionType: string, options?: CacheKeyOptions): string {
    return this.createKey('projection', projectionType, projectionName, options);
  }
   ...
   * @returns {string} 模式键
   */
  createPatternKey(namespace: string, type: string, pattern: string, options?: CacheKeyOptions): string {
    const parts = [
      options?.namespace || this.config.defaultNamespace,
      namespace,
      type,
  ...
  private truncateKey(key: string): string {
    ...
    const middleLength = this.config.maxKeyLength - prefixLength - suffixLength - 3; // 3 for "..."

    const prefix = key.substring(0, prefixLength);
    const suffix = key.substring(key.length - suffixLength);
    const hash = this.simpleHash(key.substring(prefixLength, key.length - suffixLength));
  ...
  private updateStats(namespace: string, type: string, key: string): void {
    ...
    this.stats.keysByNamespace[namespace] = (this.stats.keysByNamespace[namespace] || 0) + 1;
    this.stats.keysByType[type] = (this.stats.keysByType[type] || 0) + 1;

    // 更新平均键长度
    const totalLength = this.stats.averageKeyLength * (this.stats.totalKeys - 1) + key.length;
