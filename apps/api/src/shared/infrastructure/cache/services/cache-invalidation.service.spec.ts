...
import { CacheInvalidationService, CacheInvalidationConfig, InvalidationStrategy, InvalidationTrigger } from './cache-invalidation.service';
...
class MockCacheKeyFactory implements ICacheKeyFactory {
  create(key: string, namespace?: string, version?: string): CacheKey {
    return {
      key,
      namespace,
      version,
  ...
  matchPattern(pattern: string, key: CacheKey): boolean {
    const keyString = this.toString(key);
    return keyString.includes(pattern.replace('*', ''));
  }
}
...
describe('CacheInvalidationService', () => {
  ...
  describe('cache invalidation', () => {
    beforeEach(async () => {
      // 设置一些测试缓存数据
      const key1 = mockKeyFactory.create('key1', 'test-namespace');
      const key2 = mockKeyFactory.create('key2', 'test-namespace');
      const key3 = mockKeyFactory.create('key3', 'other-namespace');
    ...
    it('should invalidate exact keys', async () => {
      const result = await service.invalidate(['test-namespace:key1', 'test-namespace:key2'], InvalidationStrategy.EXACT);
    ...
    it('should invalidate by namespace', async () => {
      const result = await service.invalidate(['test-namespace'], InvalidationStrategy.NAMESPACE);
    ...
    it('should invalidate batch', async () => {
      const result = await service.invalidate(['test-namespace:key1', 'test-namespace:key2', 'other-namespace:key3'], InvalidationStrategy.BATCH);
    ...
    it('should handle invalidate with non-existent keys', async () => {
      const result = await service.invalidate(['non-existent-key'], InvalidationStrategy.EXACT);
    ...
    it('should handle invalidation errors gracefully', async () => {
      // 模拟缓存服务错误
      jest.spyOn(mockCacheService, 'delete').mockRejectedValue(new Error('Cache error'));

      const result = await service.invalidate(['key1'], InvalidationStrategy.EXACT);
  ...
  describe('rule-based invalidation', () => {
    it('should invalidate by rule', async () => {
      ...
      // 设置测试缓存数据
      const key = mockKeyFactory.create('key1', 'test-namespace');
    ...
    it('should handle non-existent rule', async () => {
      await expect(service.invalidateByRule('non-existent-rule')).rejects.toThrow(
        'Invalidation rule not found: non-existent-rule'
      );
    });
  });
  ...
  describe('statistics', () => {
    ...
    it('should update stats after invalidation', async () => {
      ...
      await service.invalidate(['test-namespace:key1'], InvalidationStrategy.EXACT);
    ...
    it('should reset stats', () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Invalidation stats reset', expect.any(String));
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should handle unsupported strategy', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('Unsupported invalidation strategy: unsupported');
    });
  });
    ...
    it('should emit rule events', () => {
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          type: 'rule_added',
          data: expect.objectContaining({ rule: expect.objectContaining({ name: 'test-rule' }) }),
        })
      );
    });
    ...
    it('should emit invalidation events', async () => {
      ...
      expect(mockEventEmitter.emit).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          data: expect.objectContaining({ result: expect.any(Object), strategy: InvalidationStrategy.EXACT }),
        })
      );
    });
  });
  ...
  describe('different strategies', () => {
    beforeEach(async () => {
      // 设置测试缓存数据
      const key1 = mockKeyFactory.create('user:1', 'users');
      const key2 = mockKeyFactory.create('user:2', 'users');
      const key3 = mockKeyFactory.create('product:1', 'products');
    ...
    it('should handle prefix invalidation', async () => {
      const result = await service.invalidate(['users'], InvalidationStrategy.PREFIX);
    ...
    it('should handle tag invalidation', async () => {
      const result = await service.invalidate(['user-tag'], InvalidationStrategy.TAG);
    ...
    it('should handle wildcard invalidation', async () => {
      const result = await service.invalidate(['user:*'], InvalidationStrategy.WILDCARD);
    ...
    it('should handle regex invalidation', async () => {
      const result = await service.invalidate(['user:\\d+'], InvalidationStrategy.REGEX);
